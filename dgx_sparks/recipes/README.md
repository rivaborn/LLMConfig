...
| `local/qwen35-122b-int4-nightly.yaml`                 | ⚠️ **SUPERSEDED 2026-08-02.** Its ghcr container resolves through `build-and-copy.sh`, which pulls `eugr/spark-vllm:latest` from Docker Hub on **every** cold load (~7 min of the 19m40s above). It was forked on a misdiagnosis — the `ShmRingBuffer` failures were a dropped SSH session wiping node memory, not an image fault. Keep only to test a 0.26.x-specific fix. |
